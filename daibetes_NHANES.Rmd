...
Sleep_data <-NHANES %>% select(Diabetes, SleepTrouble, Age, BMI, PhysActive, Education, Race1, Depressed, Poverty) %>% filter(!is.na(Diabetes),!is.na(SleepTrouble), !is.na(Age), !is.na(BMI), !is.na(Education), !is.na(Race1),  !is.na(Depressed), !is.na(Poverty))
summary(Sleep_data)
...
This step predicts the probability of having diabetes for each person in the dataset. By grouping by SleepTrouble and taking the mean, you get something like:

SleepTrouble = No → 8.7% probability

SleepTrouble = Yes → 14.2% probability

This indicates that, after adjusting for all factors, having sleep problems is associated with a higher probability of having Type 2 Diabetes.
