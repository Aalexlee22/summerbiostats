...
Sleep_data %>%  ggplot(aes(x=SleepTrouble, fill= Diabetes )) + geom_bar(position="fill") + scale_y_continuous(labels = scales::percent) + 
  labs(
  y = "Proportion", 
...
Sleep_data %>%
  ...
  geom_bar(position = "fill") +
  facet_wrap(~ Gender) +
  labs(title = "Prevalence of Type II Diabetes by Sleep Trouble and gender",
    x = "sleep trouble",
    y = "count",
    fill = "Diabetes",
    caption = "Figure 11") +
...
```{r}
Sleep_data %>%
  select(Diabetes, SleepTrouble, Age, BMI, PhysActive, Education, Race1, Depressed, Poverty, Gender) %>%
  tbl_summary(by = Diabetes, statistic = all_continuous() ~ "{mean} ({sd})", 
              missing = "no") %>%
  add_overall() %>%
  bold_labels()
...
After adjusting for multiple factors such as age, BMI, physical activity, race, education, poverty, and depression, individuals with sleep trouble show a higher probability of having Type II diabetes. The predicted probability increases from 8.7% to 14.2%, suggesting a moderate but meaningful association. These findings align with prior research linking sleep disorders with metabolic dysregulation and diabetes risk.
